...
allocator_sorted_list::allocator_sorted_list(size_t space_size,
	...
	if (space_size <= allocator_metadata_size) {
		if (logger) logger->error("Not enough space.");
		throw std::bad_alloc();
	}

	_trusted_memory = parent_allocator ? parent_allocator->allocate(space_size) : ::operator new(space_size);
	...
	*reinterpret_cast<fit_mode*>(memory) = allocate_fit_mode;
	memory += sizeof(fit_mode);

	*reinterpret_cast<size_t*>(memory) = space_size;
	memory += sizeof(size_t);

	new (memory) std::mutex();
	...
	*reinterpret_cast<void**>(memory) = first_block;
	if (logger) {
		logger->debug("Allocator created with size: " + std::to_string(space_size));
		logger->information("Available memory: " + std::to_string(available_size - block_metadata_size));
		logger->debug("Memory state: avail " + std::to_string(available_size - block_metadata_size));
	}
}

allocator_sorted_list::~allocator_sorted_list() {
	auto log = get_logger();
	if (log) log->debug("Allocator destructor called");

	auto memory = static_cast<char*>(_trusted_memory);
	memory += sizeof(logger*);

	auto parent_allocator = *reinterpret_cast<std::pmr::memory_resource**>(memory);
	memory += sizeof(std::pmr::memory_resource*);
	memory += sizeof(fit_mode);
...
allocator_sorted_list::allocator_sorted_list(allocator_sorted_list&& other) noexcept {
	get_logger()->debug("Constructor of allocator with moving resources called.");
...
void* allocator_sorted_list::do_allocate_sm(size_t size) {
	...
	if (logger) logger->debug("Allocation started with size " + std::to_string(size));

	auto memory = static_cast<char*>(_trusted_memory);
	memory += sizeof(void*) + sizeof(std::pmr::memory_resource*) + sizeof(fit_mode) + sizeof(size_t);

	auto* mutex_ptr = reinterpret_cast<std::mutex*>(memory);
	std::lock_guard<std::mutex> lock(*mutex_ptr);

	fit_mode mode = *reinterpret_cast<fit_mode*>(static_cast<char*>(_trusted_memory) +
												 sizeof(void*) + sizeof(std::pmr::memory_resource*));
	...
	if (adjusted_size % alignof(std::max_align_t) != 0) {
		...
		if (logger) logger->warning("Size adjusted from " +
									std::to_string(size) + " to " + std::to_string(adjusted_size));
	}
		...
		case fit_mode::first_fit: {
			for (auto it = free_begin(); it != free_end(); ++it) {
				if (it.size() >= adjusted_size) {
					selected_block = *it;
					if (it.size() <= adjusted_size + block_metadata_size + 8) use_exact_block = true;
	...
	if (!selected_block) {
		if (logger) logger->error("Failed to allocate " + std::to_string(adjusted_size) + " bytes: no suitable block found");
	...
	if (use_exact_block || block_size < adjusted_size + block_metadata_size + 8) {
		if (logger) logger->information("Using entire block of size " +
										std::to_string(block_size) + " for allocation of " +
										std::to_string(adjusted_size) + " bytes");
	...
	*reinterpret_cast<void**>(selected_block) = nullptr;
	...
	if (logger) {
		...
		logger->information("Available memory after allocation: " + std::to_string(available_memory));

		std::stringstream state;
		for (auto it = begin(); it != end(); ++it) {
			state << (it.occupied() ? "occupied " : "available ") << it.size() << " | ";
		}

		logger->debug("Memory state: " + state.str());
...
void allocator_sorted_list::do_deallocate_sm(void* at) {
	auto log = get_logger();
	if (log) log->debug("Deallocation started");

	if (!at) {
		if (log) log->warning("Nothing to deallocate");
		return;
	}
	...
	if (block_ptr < mem_start || block_ptr >= mem_end) {
		if (log) log->error("Attempt to deallocate memory not owned by this allocator");
	...
	if (next_free && static_cast<char*>(block_ptr) + block_metadata_size + block_size == next_free) {
	...
	if (log) {
		...
		log->information("Available memory after deallocation: " + std::to_string(available_memory));
		std::stringstream state;
		for (auto it = begin(); it != end(); ++it) {
			state << (it.occupied() ? "occupied " : "available ") << it.size() << "|";
		}

		log->debug("Memory state: " + state.str());
		log->debug("Deallocation finished");
	}
}

bool allocator_sorted_list::do_is_equal(const std::pmr::memory_resource& other) const noexcept {
	auto logger = get_logger();
	if (logger) logger->debug("Do_is_equal called");

	if (this == &other) return true;

	auto* derived = dynamic_cast<const allocator_sorted_list*>(&other);
	return derived && _trusted_memory == derived->_trusted_memory;
}
...
inline void allocator_sorted_list::set_fit_mode(allocator_with_fit_mode::fit_mode mode) {
	...
	*reinterpret_cast<fit_mode*>(memory) = mode;
	if (log) {
		log->information("Fit_mode changed to " + std::to_string(static_cast<int>(mode)));
		log->debug("Set_fit_mode finished");
	}
}
...
std::vector<allocator_test_utils::block_info> allocator_sorted_list::get_blocks_info() const noexcept {
	...
	if (log) log->debug("Get_info finished");
...
bool allocator_sorted_list::sorted_iterator::occupied() const noexcept {
	...
	while (free_block) {
		if (free_block == _current_ptr) return false;
